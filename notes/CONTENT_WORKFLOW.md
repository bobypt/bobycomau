...
1. Capture ideas in per-article folders under `notes/ai/`, for example:
   - `notes/ai/1_1_basics/notes.md`
   - `notes/ai/1_1_basics/linkedin/...`
...
## LinkedIn post planning (per-article)

LinkedIn content is managed alongside each article under `notes/ai/<article>/linkedin/`.

- Each file represents **one LinkedIn post plan**.
- Example: `notes/ai/1_1_basics/linkedin/2025-01-01-ai-basics-summary.md`
...
1. Draft an article outline in `notes/ai/<article>/notes.md`.
2. Flesh it out and move it into `_ai/` with `status: draft` or `published`.
3. Create 3–5 `notes/ai/<article>/linkedin/*.md` drafts linked via `related_ai_post`.
